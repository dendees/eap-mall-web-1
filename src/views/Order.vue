 ...
 * 加载订单列表
 * 函数说明：
 * - 调用 eap-mall 标准 API 获取订单分页
 * - 直接使用返回中的 items 字段渲染商品项
 * - 处理分页拼接与加载完成标识
 */
const loadData = async () => {
  const res = await getOrderList({ pageNumber: state.page, status: state.status })
  // res 结构由 utils/request.js 处理为 { code, data, msg }
  const { list, totalPage } = res.data || { list: [], totalPage: 0 }
  state.list = state.list.concat(list)
  state.totalPage = totalPage
  state.loading = false;
  if (state.page >= totalPage) state.finished = true
}

/**
 * 切换 Tab，触发刷新
 * @param {Object} tab - van-tabs 回调参数，包含 name
 */
